...
"""
Functions for evaluating the performance of the DOU sorter by relevance.
Copyright (C) 2020  Henrique S. Xavier
...
def plot_fancy_recall_curve(y_true, y_pred_rank, color='firebrick'):
    """
    Given a list of true label `y_true` and a predicted ranking for them, 
    `y_pred_rank`, plot the cumulative recall as a function of fraction of 
    the examples (in descending ranking).
    ...
    pl.plot(x, y_perfect * 100, color='0.3', label='Exact ordering')
    pl.plot(x, y * 100, color=color, label='Model ordering')
    pl.plot(x, x, color='0.3', linestyle='--', label='Random ordering')
    pl.tick_params(labelsize=14)
    pl.xlabel('% of ordered articles', fontsize=14)
    pl.ylabel('% of relevant articles', fontsize=14)
    #pl.title('Seção 1 do DOU', fontsize=14)
    pl.grid()
